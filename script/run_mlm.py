...
from dataset.dataset import DataCollatorForLanguageModeling4SimCSE
...
def main(dataset_dir, **kwargs):
    ...
    conf =  deep_update(parser_yaml('config/transformers.yaml'), parser_yaml('config/mlm.yaml'))
    conf['data']['train_file'] = os.path.join(dataset_dir, 'item_desc.txt')
    ...
    if kwargs is not None:
        ...
        conf['data'].update({k: v for k, v in kwargs.items() if k in conf['data']})
        conf.update({k: v for k, v in kwargs.items() if k in conf})
    conf['data']['train_file'] = os.path.join(dataset_dir, 'item_desc.txt')

    # assert (torch.cuda.device_count() == 1 and conf['data']['max_seq_length'] == 256) or \
    #     (torch.cuda.device_count() == 2 and conf['data']['max_seq_length'] == 512), 'Please run this script with given configuration.'
    assert (torch.cuda.device_count() == 1 and conf['data']['max_seq_length'] == 256), 'Please run this script with given configuration.'


    # basename = f'maxseqlen{conf["data"]["max_seq_length"]}_perdevicebs{conf["train"]["per_device_train_batch_size"]}'
    # conf['train']['output_dir'] = os.path.join(conf['train']['output_dir'], basename)
    basename = f'maxseqlen{conf["data"]["max_seq_length"]}_perdevicebs{conf["train"]["per_device_train_batch_size"]}'
    if conf['simcse']:
        basename += f'_temp{conf["temperature"]}_mlmweight{conf["mlm_weight"]}'
    conf['train']['output_dir'] = os.path.join(conf['train']['output_dir'], basename)


    training_args = TrainingArguments(**conf['train'])
    model_args = ModelArguments(**conf['model'])
    data_args = DataTrainingArguments(**conf['data'])
    ...
    config.mlm = conf['mlm']
    config.simcse = conf['simcse']
    config.temperature = conf['temperature']
    config.score_fn = conf['score_fn']
    config.mlm_weight = conf['mlm_weight']
    ...
    data_collator = DataCollatorForLanguageModeling4SimCSE(
        simcse=conf['simcse'],
        tokenizer=tokenizer,
