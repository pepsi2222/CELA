...
class Dice(torch.nn.Module):
    ...
    def __init__(self, num_parameters, init: float = 0.0, epsilon: float = 1e-08):
        ...
        # self.batch_norm = torch.nn.BatchNorm1d(num_parameters)
        self.batch_norm = torch.nn.BatchNorm1d(num_parameters, epsilon, affine=False)

    def forward(self, x):
        # mean_x = torch.mean(x, dim=-1, keepdim=True)
        # var_x = torch.var(x, dim=-1, keepdim=True)
        # x_std = (x - mean_x) / (torch.sqrt(var_x + self.epsilon))
        # x_norm = self.batch_norm(x_std)
        # p_x = torch.sigmoid(x_norm)
