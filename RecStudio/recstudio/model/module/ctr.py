...
class DenseEmbedding(torch.nn.Module):
    ...
    def forward(self, input):
        ...
        emb = emb.view((*shape, self.embedding_dim))
        return emb
...
class SelfAttentionInteractingLayer(nn.Module):
    ...
    def __init__(self, embed_dim, activation, n_head=1, dropout=0, residual=True, residual_project=True, layer_norm=False):
        super().__init__()
        self.embed_dim = embed_dim
        self.n_head = n_head
        self.dropout = nn.Dropout(dropout)
    ...
    def extra_repr(self):
        return f"attention_dim={self.embed_dim}, n_head={self.n_head}, dropout={self.dropout.p}, residual={self.residual}, layer_norm={self.layer_norm}"
...
class SerialMaskNet(nn.Module):
    ...
    def forward(self, inputs):
        # inputs : [B, M, D]
        bs = inputs.size(0)
        block_out = self.layer_norm(inputs)
        for mb in self.mask_blocks:
            block_out = mb(inputs.flatten(1), block_out.flatten(1))
